...
function ExcelUploadSection() {
  ...
  void fileRef; // unused suppression
...
function ProductImageCard({ product }: { product: Product }) {
  const src = product.imageFile ? `/images/${encodeURIComponent(product.imageFile)}` : null;
  const [ok, setOk] = useState<boolean | null>(src ? null : false);

  return (
    <div className={`bg-white rounded-xl border overflow-hidden flex flex-col ${ok === false ? "border-red-100" : "border-gray-100"}`}>
      ...
      <div className="p-2">
        ...
        <p className={`text-xs mt-1 font-medium ${ok === false ? "text-red-400" : ok === true ? "text-emerald-500" : "text-gray-300"}`}>
          {ok === false ? "이미지 없음" : ok === true ? product.imageFile : "확인 중..."}
        </p>
      </div>
    </div>
  );
        ...
        <div className="max-w-6xl mx-auto px-4 py-3 flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-400">
              ...
              <span className="ml-2 text-gray-300">· 이미지 추가 후 배포하면 반영</span>
            </p>
          </div>
